...
class DataModel:
    ...
    def __init__(self):
        ...
        self.df = pd.read_csv('data_training_2.csv')
        X = self.df.iloc[:,1:26] #input features (GAO data criteria)
        print(f'X: {X}')
        ...
        while not done:
            ...
            self.model = Sequential([ #two hidden layers followed by output layer
                Dense(units=32, activation='relu', input_shape=(25,)),
            ...
            """
            hist = self.model.fit(X_train, Y_train, batch_size=32, epochs=75, validation_split=0.1, validation_data=(X_val, Y_val))
